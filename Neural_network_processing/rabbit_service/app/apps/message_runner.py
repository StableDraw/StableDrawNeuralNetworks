...
def bool_converter(params: dict):
...
async def run_neurals(msg):
    msg_response = {}
    neural_type = msg['neuralType']
    params = bool_converter(json.loads(msg['parameters']))
    print(params)
    try:
        if neural_type == 'colorizer':                                    
            ...
            result = INeuralProxy.upscaler(
                init_img_binary_data=base64_to_bytes(msg['imagesInput'][0]), caption=str(msg['caption']), params=params)            
            msg_response = gen_message(
                    msg['orderId'], neural_type, images=[bytes_to_base64(result)])
            ...
            msg_response = gen_message(msg['orderId'], neural_type, images=[bytes_to_base64(result[0])]) #��������� �������
            


        elif neural_type == 'text_to_image':
            result = INeuralProxy.text_to_image(
                caption=msg['caption'], params=params)  
            ...
            result = INeuralProxy.inpainting(
                init_img_binary_data=base64_to_bytes(msg['imagesInput'][0]), mask_binary_data=base64_to_bytes(msg['imagesInput'][1]), caption=msg['caption'], params=params)            
            msg_response = gen_message(
                msg['orderId'], neural_type, images=array("i", result))
        elif neural_type == 'stylization':
            result = INeuralProxy.stylization(
                content_binary_data=base64_to_bytes(msg['imagesInput'][0]), caption=msg['caption'], style_binary_data=base64_to_bytes(msg['imagesInput'][1]), params=params)            
            msg_response = gen_message(
                msg['orderId'], neural_type, images=array("i", result))
        elif neural_type == 'image_fusion':
            result = INeuralProxy.image_fusion(img1_binary_data=base64_to_bytes(msg['imagesInput'][0]), img2_binary_data=base64_to_bytes(msg['imagesInput'][1]),
             caption1=msg['prompts'][0], caption2=msg['prompts'][1], params=params)            
            msg_response = gen_message(
                msg['orderId'], neural_type, images=array("i", result))
        ...
        # traceback.print_exc(ex.__traceback__())
        # print("Hello")
        print(ex.__str__())
        msg_response = gen_message(
            msg['orderId'], neural_type, errorMsg="Error in Neural service")
    finally:
        return msg_response
