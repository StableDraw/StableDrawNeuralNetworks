...
from ...modules.diffusionmodules.sampling_utils import (get_ancestral_step, linear_multistep_coeff, to_d, to_neg_log_sigma, to_sigma)
...
class BaseDiffusionSampler:
    def __init__(self, discretization_config: Union[Dict, ListConfig, OmegaConf], num_steps: Union[int, None] = None, guider_config: Union[Dict, ListConfig, OmegaConf, None] = None, verbose: bool = False, device: str = "cuda"):
    ...
    def get_sigma_gen(self, num_sigmas):
        ...
        if self.verbose:
            ...
            print(f"             : {self.discretization.__class__.__name__}")
            print(f"         : {self.guider.__class__.__name__}")
            sigma_generator = tqdm(sigma_generator, total = num_sigmas - 1, desc = f"                   {self.__class__.__name__}     {num_sigmas - 1}      ")
...
class AncestralSampler(SingleStepDiffusionSampler):
    ...
    def ancestral_step(self, x, sigma, next_sigma, sigma_up):
        x = torch.where(append_dims(next_sigma, x.ndim) > 0.0, x + self.noise_sampler(x) * self.s_noise * append_dims(sigma_up, x.ndim), x)
        return x
...
class DPMPP2MSampler(BaseDiffusionSampler):
    ...
    def sampler_step(self, old_denoised, previous_sigma, sigma, next_sigma, denoiser, x, cond, uc = None):
        denoised = self.denoise(x, denoiser, sigma, cond, uc)
    ...
    def __call__(self, denoiser, x, cond, uc = None, num_steps = None, **kwargs):
        ...
        old_denoised = None
        for i in self.get_sigma_gen(num_sigmas):
            x, old_denoised = self.sampler_step(old_denoised, None if i == 0 else s_in * sigmas[i - 1], s_in * sigmas[i], s_in * sigmas[i + 1], denoiser, x, cond, uc=uc)
        return x
